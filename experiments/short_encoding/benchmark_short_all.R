...
wilcox.test(
  results[which(results$algorithm=="AgingEvol" & results$epochs==36 & results$init=="lhs"), c("test_accuracy")],
  results[which(results$algorithm=="AgingEvol" & results$epochs==36 & results$init=="centroids"), c("test_accuracy")])
  ...
  for(init in c("rand", "centroids", "lhs")) {
    for(epochs in c(36, 108)) {
      for(i in 0:104) {
        ...
        meanLog <- rbind(meanLog, data.frame(algorithm=alg, init=init, epochs=epochs, ngen=i * 19, mean=mean(tmp$mean), std=sd(tmp$mean), max=mean(tmp$max), stdmax=sd(tmp$max),  
                                             min=mean(tmp$min)) )
...
p <- p + theme_bw() + facet_wrap(~epochs + algorithm, nrow=2) + theme(text=element_text(size=20))
p <- p + ylab("Mean population validation accuracy") + xlab("Step")
p <- p + geom_ribbon(aes(x=ngen, y=mean, ymin=(mean-std), ymax=(mean+std), fill=init), alpha=0.1) + ylim(0.80, 0.95)
p


p1 <- ggplot(meanLog, aes(x=ngen, y=mean)) + geom_line(aes(color=init))#, lwd = 1.25)
p1 <- p1 + theme_bw() + facet_wrap(~epochs + algorithm, nrow=2) + theme(text=element_text(size=20))
p1 <- p1 + ylab("Population validation accuracy") + xlab("Step")
p1 <- p1 + geom_ribbon(aes(x=ngen, y=mean, ymin=min, ymax=max, fill=init), alpha=0.1) + ylim(0.85, 0.95)
p1
